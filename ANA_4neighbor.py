...
def successor(mazelist,s):
    ...
    for i in range (0,len(mazelist)):
        if i!=s:#This condition is to avoid considering the parent node as a child. The condition below is to satisfy the 4 neighbor for a parent node condition
            if ((mazelist[i].x==mazelist[s].x and mazelist[i].y==mazelist[s].y+1) or (mazelist[i].x==mazelist[s].x and mazelist[i].y==mazelist[s].y-1) or (mazelist[i].x==mazelist[s].x+1 and mazelist[i].y==mazelist[s].y) or (mazelist[i].x==mazelist[s].x-1 and mazelist[i].y==mazelist[s].y)):
                    if mazelist[i].color!="1":#This condition checks if the child is an obstacle or not. If obstacle this child will be ignored
...
def ana_star(maze, start_node, exit_node):
    global mazelist,G,E,openlist,Gnode
    ...
    while len(openlist)!=0:#This block of code executes the ANA* ideologies
        Gnode.append(improvesolution(exit_node))
